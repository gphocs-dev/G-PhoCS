...
void appendCurrent(int comb, int currentPop, int gene){
	...
	for ( ; event >= 0 ; i++, event = event_chains[gene].events[event].getNextIdx()){
		eventAge += event_chains[gene].events[event].getElapsedTime();
		currentStats->sorted_ages[i] = eventAge;
		currentStats->elapsed_times[i] = event_chains[gene].events[event].getElapsedTime();
		currentStats->num_lineages[i] = event_chains[gene].events[event].getNumLineages();
		currentStats->event_types[i] = event_chains[gene].events[event].getType();
	}
