...
// UNUSED  numZero=0;
  ...
  {
    fprintf(stderr,
            "Error: Number of loci read from rate file (%d), was less than the"
            " number of loci (%d) specified in the sequence file.\n",
...
//        }
        ...
        event_chains[gen].events[\
          locus_data[gen].mig_spr_stats.father_event_new].type = COAL;
        event_chains[gen].events[\
          locus_data[gen].mig_spr_stats.father_event_new].node_id = father;
...
//    for( gen = 0; gen < dataSetup.numLoci; ++gen )
//    {
//      checkAffectedMigBandsInGen(gen, affected_mig_bands,
//                                 start_or_end, num_affected_mig_bands);
//    }
//    printf("UpdateTau (T %d) %d\n", omp_get_thread_num(), c);
        ...
        else
        {
          ...
          for (i = 0; i < num_affected_mig_bands; i++)
          {
            ...
            for (event_mt = event_chains[gen].first_event[targetPop_mt];
                 event_mt >= 0;
                 event_mt = event_chains[gen].events[event_mt].next)
            {
              if( event_chains[gen].events[event_mt].node_id == mig_band
                  && ((event_chains[gen].events[event_mt].type == MIG_BAND_START
                       && start_or_end[i])
                      ||
                      event_chains[gen].events[event_mt].type == MIG_BAND_END))
                break;
            }
            if (event_mt < 0)
            {
      ...
      for (gen = 0; gen < dataSetup.numLoci; gen++)
      {
        ...
        for (i = 0; i < locus_data[gen].rubberband_migs.num_moved_events; i++)
        {
          ...
          int mig = event_chains[gen].events[locus_data[gen].\
                                        rubberband_migs.new_events[i]].node_id;
          if( event_chains[gen].events[locus_data[gen].\
               rubberband_migs.new_events[i]].type == IN_MIG )
          {
            ...
            genetree_migs[gen].mignodes[mig].age =
                locus_data[gen].rubberband_migs.new_ages[i];
                      ...
                      rubberband_migs.new_events[i]].type == OUT_MIG )
          {
...
#pragma omp parallel for private(gen) schedule(THREAD_SCHEDULING_STRATEGY)
      ...
      for (gen = 0; gen < dataSetup.numLoci; gen++)
      {
        ...
        for( i = 0; i < locus_data[gen].rubberband_migs.num_moved_events; ++i )
        {
          ...
          if( event_chains[gen].events[nRubMigNEvIdx].type == IN_MIG )
          {
