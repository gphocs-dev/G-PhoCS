/** 
   \file LocusGenealogy.c 
   Data structures and functions to hold info about a locus genealogy.

	
   Contains the relevant data structures and procedure implementations
   for holding detailed information about a locus genealogy.
 ...
 *		Note: mostly next pointer is used
 *		Note: POP_END event points up to POP_START event of father population
 *		Note: POP_START events do not point to prev
...
struct EVENT {							
  ...
  Event *next, *prev;		// pointers to next and previous intervals in chain
  ...
  int auxId;				// auxiliary id:
							...
							//	-for MIG_BAND_START/MIG_BAND_END the id of the migration band
};								
 ...
 *	- For each branch we hold information on the node below it:
...
/***********************************************************************************
 *	GenStats
 *	- Data type which holds statistics for locus genealogy for quick computation of 
 *		likelihood of genealogy (Pr[G_i | PopTree])
 ***********************************************************************************/
...
/***********************************************************************************
 *	GenChanges
 *	- Data type which holds changes made to genealogy due to proposed modification
 *		to accommodate quick acceptance/rejection of change
 ***********************************************************************************/
typedef struct GEN_CHANGES{
  ...
  unsigned short prunedChildOfEvent;	// indicating whether pruning was done on sides
										// 0 or 1 of prunedCoalEvent
...
int generateRandomGenealogy(LocusGenealogy* locusGen) {
  ...
  // attach rest of leaves one by one
 ...
 *	- propose a genealogy modification which re-coalesces a pruned subtree.
...
int migrateAndCoalesceLineage(LocusGenealogy* locusGen, int branchId) {
  ...
  // in an empty genealogy (when generating a random genalogy).
...
int computeNewGenStats(LocusGenealogy* locusGen) {
  ...
  // consider numer-of-lineages changes and update coal and mig stats accordingly
