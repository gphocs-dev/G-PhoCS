...
LocusEmbeddedGenealogy::LocusEmbeddedGenealogy(
        ...
        : genealogy_(pDataSetup->numSamples), //construct genealogy
          intervals_(locusID, numIntervals, pDataSetup->popTree),  //construct intervals
...
#pragma omp atomic
            ...
            default:
                if (debug) {
                    std::cout
                    << endl << "Error: recalcStats: event of unknown type "
                    << pInterval->typeToStr() << " in population " << pop
                    << " gen "  << locusID_ << endl;
...
void LocusEmbeddedGenealogy::testPopIntervals() {
    ...
    for (int pop = 0; pop < dataSetup.popTree->numPops; pop++) {
        ...
        //iterate both old (events) and new (intervals) structures
        for (event; event >= 0; event = event_chains[locusID_].events[event].getNextIdx()) {
