...
//	ioSetup.debugFile = fopen(ioSetup.debugFileName,"w");
  ...
  for (iteration = -mcmcSetup.burnin; iteration < mcmcSetup.numSamples;
    ...
    if (!checkAll())
    {
      fprintf(stderr, "\nError:  --  Aborting after AdmixtureCoefficients "
              "in MCMC iteration %d.\n\n",iteration);
      exit(-1);
    }
#endif
...
//		  @@ronv: please enter here :)
      ...
      if (isTauBoundsActivated()) {
        ...
        calculateReferenceMigrationStats();
        printReferenceMigrationStats(iteration, ioSetup.refMigStatsFile); // TODO - put real file
      }
...
[admixture_status.sampledLoci[gen]]
    ...
    if ((iteration + 1) % numSamplesPerLog == 0) {
      ...
      acceptancePercents.migRate = acceptanceCounts.migRate * 100.0 /
                                   (((double) logCount) *
                                    dataSetup.popTree->numMigBands + 0.000001);
      for (pop = 0; pop < dataSetup.popTree->numPops; pop++) {
      ...
      if (findingFinetunes) {
        if (acceptancePercents.coalTime >
          ...
          if (finetuneMaxes.coalTime - finetuneMins.coalTime <
              FINETUNE_RESOLUTION) { // recompute maximal finetune
//							finetuneMaxes.coalTime = MAX_FINETUNE;
          ...
          if (finetuneMaxes.coalTime - finetuneMins.coalTime <
              FINETUNE_RESOLUTION) { // recompute minimal finetune
//							finetuneMins.coalTime = 0.0;
...
//							finetuneMins.migRate = 0.0;
        ...
        if (acceptancePercents.admix >
            TARGET_ACCEPTANCE_PERCENT + TARGET_ACCEPTANCE_RANGE) {
          //raise the minimal finetune value
          finetuneMins.admix = mcmcSetup.finetunes.admix;
          if (finetuneMaxes.admix - finetuneMins.admix <
              FINETUNE_RESOLUTION) { // recompute maximal finetune
//							finetuneMaxes.migRate = MAX_FINETUNE;
            if (finetuneMaxes.admix >= MAX_FINETUNE) {
              ...
              finetuneMaxes.admix *= 2.0;
                   ...
                   TARGET_ACCEPTANCE_PERCENT - TARGET_ACCEPTANCE_RANGE) {
          //lower the maximal finetune value
          finetuneMaxes.admix = mcmcSetup.finetunes.admix;
          if (finetuneMaxes.admix - finetuneMins.admix <
              FINETUNE_RESOLUTION) { // recompute minimal finetune
//							finetuneMins.admix = 0.0;
        ...
        //finetune set to midpoint of interval
        mcmcSetup.finetunes.admix = 0.5 * (finetuneMaxes.admix +
                                           finetuneMins.admix);
...
void setupMcRefFiles() {
  ...
  if (isTauBoundsActivated()) {
    ...
    ioSetup.refMigStatsFile = fopen(ioSetup.refMigStatsFileName, "w");
    if (ioSetup.refMigStatsFile == NULL) {
      fprintf(stderr, "Error: Could not open refMigStatsFile %s.\n", ioSetup.refMigStatsFileName);
      exit(0);
    }
    printRefMigStatsHeader(ioSetup.refMigStatsFile);
...
#pragma omp parallel for private(gen) schedule(THREAD_SCHEDULING_STRATEGY)
  ...
  for (gen = 0; gen < dataSetup.numLoci; gen++) {
    ...
    for (m = 0; m < genetree_migs[gen].num_migs; m++) {
      ...
      t_bounds[0] = dataSetup.popTree->migBands[genetree_migs[gen].\
                                   mignodes[mignode].migration_band].startTime;
      t_bounds[1] = dataSetup.popTree->migBands[genetree_migs[gen].\
...
#pragma omp atomic
        ...
        for (mig_band = 0;
             mig_band < dataSetup.popTree->numMigBands;
             ++mig_band) {
...
int UpdateTheta(double finetune) {
  ...
  for (pop = 0; pop < dataSetup.popTree->numPops; pop++) {
    ...
    lnacceptance =
        lnc + lnc * (dataSetup.popTree->pops[pop]->thetaPrior.alpha - 1) -
        (thetanew - thetaold) * dataSetup.popTree->pops[pop]->thetaPrior.beta;
...
int UpdateMigRates(double finetune) {
  ...
  for (mig_band = 0; mig_band < dataSetup.popTree->numMigBands; mig_band++) {
    ...
    lnacceptance = lnc + lnc *
                         (dataSetup.popTree->migBands[mig_band].\
...
void UpdateTau(double *finetunes, int *accepted) {
  ...
  for (ancestralPop = dataSetup.popTree->numCurPops;
       ancestralPop < dataSetup.popTree->numPops;
       ++ancestralPop) {
      ...
      if ((sourcePop == sons[0] && targetPop == sons[1]) ||
        ...
        if (dataSetup.popTree->migBands[mig_band].endTime < taub[1]) {
          ...
          new_band_ages[num_affected_mig_bands] = taub[1] +
                                                  (dataSetup.popTree->migBands[mig_band].endTime - taub[1])
                                                  / taufactor[1];
        ...
        if (dataSetup.popTree->migBands[mig_band].startTime<taub[1]
                                                            && dataSetup.popTree->pops[sourcePop]->age>min2(tauold,
                                                                                                            taunew)) {
          affected_mig_bands[num_affected_mig_bands] = mig_band;
        ...
        if (dataSetup.popTree->migBands[mig_band].endTime > taub[0] &&
            dataSetup.popTree->pops[sourcePop]->father->age <
            max2(tauold, taunew)) {
          affected_mig_bands[num_affected_mig_bands] = mig_band;
          //indicate that end time has changed
          start_or_end[num_affected_mig_bands] = 0;
        ...
        for (i = 0; i < genetree_migs[gen].num_migs; i++) {
          if (mig_conflict == 0) {
            ...
            if (event_mt >= 0) {
              ...
              // check for conflicts
              if (new_age_mt >= dataSetup.popTree->migBands[mig_band].endTime) {
                mig_conflict = 1;
#ifdef ENABLE_OMP_THREADS
#pragma omp flush (mig_conflict)
#endif
  ...
  for (pop = 0; pop < dataSetup.popTree->numCurPops; pop++) {
    ...
    for (mig_band = 0; mig_band < dataSetup.popTree->numMigBands; ++mig_band) {
      ...
      if (targetPop == pop) {
        ...
        // so that after rubber-band they will be in the right spot
        if (dataSetup.popTree->migBands[mig_band].endTime < taub[1]
            && dataSetup.popTree->migBands[mig_band].endTime > taub[0]) {
          //printf("    mig band %d, type 1a.\n",mig_band);
          affected_mig_bands[num_affected_mig_bands] = mig_band;
          //indicate that end time has changed
          start_or_end[num_affected_mig_bands] = 0;
        ...
        if (dataSetup.popTree->migBands[mig_band].startTime < taub[1]
            && dataSetup.popTree->migBands[mig_band].startTime > taub[0]) {
...
int mixing(double finetune) {
  ...
  lnacceptance = lnc * (2 * dataSetup.popTree->numPops -
                        dataSetup.popTree->numCurPops -
  ...
  for (pop = 0; pop < dataSetup.popTree->numPops; pop++) {
    ...
    lnacceptance += lnc
                    * (dataSetup.popTree->pops[pop]->thetaPrior.alpha - 1)
                    - (xnew - xold)
                      * dataSetup.popTree->pops[pop]->thetaPrior.beta;
  ...
  for (pop = dataSetup.popTree->numCurPops;
    ...
    lnacceptance += lnc * (dataSetup.popTree->pops[pop]->agePrior.alpha - 1) -
                    (xnew - xold) * dataSetup.popTree->pops[pop]->agePrior.beta;
  }
  for (mig_band = 0; mig_band < dataSetup.popTree->numMigBands; mig_band++) {
    ...
    // GAMMA PRIOR
    lnacceptance +=
        -lnc * (dataSetup.popTree->migBands[mig_band].migRatePrior.alpha - 1) -
        (xnew - xold) * dataSetup.popTree->migBands[mig_band].migRatePrior.beta;
...
#endif
    if (lnacceptance >= 0
        || rndu(RAND_GENERAL_SLOT) < exp(lnacceptance)) { /* accept */
#ifdef LOG_STEPS
      fprintf(ioSetup.debugFile, "accepting.\n");
#endif
...
#pragma omp parallel for private(gen) schedule(THREAD_SCHEDULING_STRATEGY)
      ...
      for (gen = 0; gen < dataSetup.numLoci; gen++) {
        ...
        for (mig_band_mt = 0;
             mig_band_mt < dataSetup.popTree->numMigBands; mig_band_mt++) {
      ...
      for (mig_band = 0;
           mig_band < dataSetup.popTree->numMigBands; mig_band++) {
